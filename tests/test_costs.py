...
from style_transfer_gs_2023.hyperparameters import LayerWeight
...
def test_content_cost():
    ...
    J_content = _content_cost(
        content_activations, generated_activations, content_layer_idx=-1
    )
    J_content_zero = _content_cost(
        content_activations, content_activations, content_layer_idx=-1
    )
...
def test_style_cost():
    ...
    layers = [LayerWeight(idx=1, weight=1)]
